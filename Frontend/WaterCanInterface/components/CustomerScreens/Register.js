...
const Register = ({route, navigation}) => {
  ...
  registerData = () => {
    if (shopname && supplierid) {
      Alert.alert(
        'Invalid data',
        'Please provide either Shop name  if you are supplier and SupplierId if you are consumer. Not both ',
        [{text: 'OK'}],
      );
      return;
    }
    if (isNaN(contactno)) {
      Alert.alert('Invalid data', 'Contact number should not contain letters', [
        {text: 'OK'},
      ]);
      return;
    }
    ...
    if (street === '') {
      Alert.alert('Invalid data', 'Street name cannot be empty', [
        {text: 'OK'},
      ]);
    }
    ...
    if (shopname) {
      axios({
        ...
        .then((responsejson) => {
          ...
          Alert.alert('Successfully registered', JSON.stringify(iddata), [
            {text: 'OK and Restart'},
          ]);
        })
        .catch((error) => {
          console.log(
            'There has been a problem in registration. ' + error.message,
          );
          Alert.alert(
            'Sorry',
            'Registration failed. Please check network or try again later',
            [{text: 'OK'}],
          );
        });
    } else if (supplierid) {
      ...
      Alert.alert(
        'Invalid data',
        'Please provide Shopname if you are supplier and SupplierId if you are consumer',
        [{text: 'OK'}],
      );
      return;
    }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text> Please get the supplier id from your can supplier</Text>
      <TextInput
        style={styles.input}
        underlineColorAndroid="transparent"
        placeholder="SupplierId ( Mandatory field) "
        placeholderTextColor="#a9a9a9"
        autoCapitalize="none"
      ...
      <View style={styles.buttonstyle}>
        ...
        />
        <Button color="#228b22" title="Submit" onPress={() => registerData()} />
      </View>
    </ScrollView>
  );
};
